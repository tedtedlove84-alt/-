...
const cleanerPeople=['난희','홍길동','김민지'];
...
function showView(id){
  if(id==='listing'&&isPortfolio()){activePropertyId=properties[0].id;localStorage.setItem('andwith-active-property',activePropertyId);renderPropertyMenu()}
...
function renderCleaningSchedules(){
  ...
  const previous=filter.value||'all';filter.innerHTML='<option value="all">전체 담당자</option><option value="unassigned">담당자 미정</option>'+cleanerPeople.map(name=>`<option value="${name}">${name}</option>`).join('');filter.value=[...filter.options].some(option=>option.value===previous)?previous:'all';
